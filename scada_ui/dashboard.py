...
# Function to access the actual water level in the tank
# This will be set by main.py to access the actual water tank model
class TrueWaterLevelGetter:
    def __init__(self):
        self.getter = lambda: 10.0  # Default value
    
    def __call__(self):
        # Return the true water level from the tank model
        return self.getter()

api_get_true_water_level = TrueWaterLevelGetter()

# Function to print current levels (for debugging)
def print_levels():
    print(f"[DEBUG] water_level (reported): {water_level:.2f}, actual_water_level: {actual_water_level:.2f}")
    print(f"[DEBUG] is_replay_active: {is_replay_active}")

# Global variables to store water level and flow rates
water_level = 10.0
actual_water_level = 10.0  # Actual physical water level (may differ during replay attack)
is_replay_active = False  # Flag to indicate if replay attack is active
tank_inflow = 0.0
tank_outflow = 0.0
    ...
    <html>
      ...
      <body>
                ...
                <div class="card-body text-center">
                  <div class="d-flex justify-content-between mb-2">
                    <div class="text-start">
                      <span class="badge bg-primary">Reported Level</span>
                    </div>
                    <div class="text-end">
                      <span class="badge bg-danger">Actual Level</span>
                    </div>
                  </div>
                  <div class="tank-container">
                    ...
                    <!-- Actual water level indicator (red line) -->
                    <div id="actual-water-line" style="position: absolute; width: 100%; height: 2px; background-color: #dc3545; bottom: 0%; z-index: 10;"></div>
                  </div>
                  <div class="d-flex justify-content-between mt-3">
                    <h3 class="text-primary">Reported: <span id="level-value">{{ level }}</span> units</h3>
                    <h3 class="text-danger">Actual: <span id="actual-level-value">{{ level }}</span> units</h3>
                  </div>
                </div>
              </div>
            </div>
        ...
        <script>
          ...
          const actualWaterLine = document.getElementById('actual-water-line');
          ...
          function updateDashboard() {
            fetch('/api/water-level')
              ...
              .then(data => {
                ...
                // Calculate height percentage for display (clamped between 0-100%)
                const reportedHeight = Math.min(Math.max(reportedLevel, 0), 100);
                const actualHeight = Math.min(Math.max(actualLevel, 0), 100);
                
                // Update reported level display
                waterElement.style.height = `${reportedHeight}%`;
                levelElement.innerText = reportedLevel.toFixed(2);
                
                // Update actual level display (red line)
                ...
                if (data.is_emergency || data.system_status === 'Emergency Stop') {
                  ...
                  // Simulate control activity (just visual feedback)
                  const controlPct = Math.min(100, Math.max(0, 50 + (reportedLevel - 50) * 2));
...
# API endpoint for setting replay attack status
@app.route('/api/set-replay-status', methods=['POST'])
def set_replay_status():
    global is_replay_active, water_level, actual_water_level, history, actual_history
    from flask import request, jsonify
    
    data = request.json
    if data and 'is_replay_active' in data:
        new_status = bool(data['is_replay_active']) 
        old_status = is_replay_active
        is_replay_active = new_status
        
        # Only take action when the status changes
        if new_status != old_status:
            status_str = "ACTIVATED" if new_status else "DEACTIVATED"
            print(f"===> REPLAY ATTACK {status_str} <===")
            print(f"     Reported level will now {'DIFFER FROM' if new_status else 'MATCH'} actual level")
            
            # If activating replay, force reported level to be very different from actual
            if new_status:
                # Get the actual level
                true_level = api_get_true_water_level()
                actual_water_level = true_level
                
                # Force reported level to be drastically different
                if true_level < 50:
                    water_level = 80.0  # Force high if actual is low
                else:
                    water_level = 20.0  # Force low if actual is high
                    
                # Update histories to match
                if history:
                    history[-1] = water_level
                if actual_history:
                    actual_history[-1] = actual_water_level
                    
                print(f"[FORCE] Set reported_level={water_level:.2f}, actual_level={actual_water_level:.2f}")
...
# API endpoint to get the true water level directly from the model
@app.route('/api/true-water-level')
def get_true_water_level():
    from flask import jsonify
    # Get true level from the tank model
    true_level = api_get_true_water_level()
    print(f"[API] Returning true water level: {true_level:.2f} units")
    return jsonify({'true_level': true_level})

# API endpoint to set reported level (for replay attack)
@app.route('/api/water-level', methods=['POST'])
def set_reported_level():
    global water_level, history
    from flask import request, jsonify
    
    data = request.json
    if data and 'set_reported_level' in data:
        new_level = float(data['set_reported_level'])
        water_level = new_level
        
        # Update history for the reported level
        if history:
            history[-1] = new_level
        else:
            history.append(new_level)
            
        print(f"[API] Set reported water level to {new_level:.2f} units")
        return jsonify({
            'status': 'success', 
            'reported_level': water_level,
...
def update_water_level(new_level, inflow=None, outflow=None, emergency_state=False, new_pressure=None, 
    ...
    global tank_inflow, tank_outflow, controller_status, outflow_status
    global system_status, is_emergency, pressure, inflow_valve_position, outflow_valve_position
    
    # Get true level directly from the tank if available
    true_level = api_get_true_water_level()
    
    # When in replay mode, we need to handle things differently:
    # 1. Keep the actual_water_level as the true physical level from the tank
    # 2. Don't update water_level, as it'll be set by the replay attack
    if is_replay_active:
        print(f"[UPDATE] Replay mode active - Keeping actual_level={true_level:.2f}, reported_level unchanged")
        actual_water_level = true_level
        # Don't update water_level; it will be set by the replay
    else:
        # In normal operation, both levels are the same
        print(f"[UPDATE] Normal mode - Setting both levels to {new_level:.2f}")
    ...
    # Update the actual level history with the true level from the tank
    actual_history.append(actual_water_level)
    
    # In normal mode, history = actual_history
    # In replay mode, history isn't updated here (it comes from the recording)
    if not is_replay_active:
        history.append(new_level)
    # Force both arrays to be the same length
    while len(actual_history) > len(history):
        # If we need to pad history (replay active but missing data)
        history.append(history[-1] if history else 0)
    
    # Ensure all arrays stay at the right size
